...
void MainThread::search() {
...
#if defined(NANOHA)
  int contempt = Options["Contempt"]; // 歩=100とする.
#else
  ...
  DrawValue[~us] = VALUE_DRAW + Value(contempt);
	...
	enum { NotFoundBest, Book, Mate, Kachi, Resign } bestType = NotFoundBest;
	...
	// 入玉勝ち宣言できるか？
	if (!foundBestMove && rootPos.IsKachi(us)) {
		foundBestMove = true;
		bestType = Kachi;
		bestMove = MOVE_WIN;
	}
  ...
  if (!foundBestMove) {
  	...
  	// 問題集を解かせたときに、指し手を返すためにPVに設定する。
  	rootMoves[0].pv.clear();
  	rootMoves[0].pv.push_back(bestMove);
  	if (bestType == Book) {
  		sync_cout << "bestmove " << UCI::move(bestMove) << sync_endl;
  		rootMoves[0].score = VALUE_ZERO;
  	} else if (bestType == Mate) {
  		sync_cout << "bestmove " << UCI::move(bestMove) << sync_endl;
  		rootMoves[0].score = VALUE_MATE;
  	} else if (bestType == Kachi) {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning) {
    ...
    if (!rootNode)
    {
        ...
        if (Signals.stop.load(std::memory_order_relaxed)
           || pos.is_draw(ss->ply, repeat_check) || ss->ply >= MAX_PLY)
...
#if defined(NANOHA)
                update_cm_stats(ss, piece_of(ttMove), to_sq(ttMove), penalty);
#else
        ...
        &&  abs(beta) < VALUE_MATE_IN_MAX_PLY)
    {
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (pos.legal(move))
            {
                ss->currentMove = move;
#if defined(NANOHA)
                const Piece pc = is_promotion(move) ? Piece(piece_of(move) | PROMOTED) : piece_of(move);
                ss->counterMoves = &thisThread->counterMoveHistory[pc][to_sq(move)];
#else
...
moves_loop: // When in check search starts from here
...
#if defined(NANOHA)
				// もし、駒を動かさずにいた場合、その駒を取られて損をするか？
				// [ToDo] 移動先の駒を取り除き、
				// 元の位置に元の駒を置く手で SEE する
				else {
					Square from = from_sq(move);
					Square to = to_sq(move);
					effect_t efft = (pos.side_to_move() == BLACK) ? pos.exist_effect<BLACK>(from) : pos.exist_effect<WHITE>(from);
					if (efft && !pos.see_ge(Move(To2Move(to) | Piece2Move(piece_of(move))),  VALUE_ZERO)) {
						r -= 2 * ONE_PLY;
					}
				}
#else
...
#if defined(NANOHA)
	int repeat_check = 0;
	if (pos.is_draw(ss->ply, repeat_check) || ss->ply >= MAX_PLY) {
        return ss->ply >= MAX_PLY ? evaluate(pos)
                                  : DrawValue[pos.side_to_move()];
	}
	if(repeat_check < 0) {
  ...
  void update_stats(const Position& pos, Stack* ss, Move move,
    ...
    update_cm_stats(ss, pc, to_sq(move), bonus);
#else
    thisThread->history.update(c, move, bonus);
    ...
    {
#if defined(NANOHA)
	const Piece qpc = is_promotion(quiets[i]) ? Piece(piece_of(quiets[i])|PROMOTED) : piece_of(quiets[i]);
