...
// 実行ファイル起動時に行う初期化.  --> Position::init()に移動
...
//	Tesu = 0;
	...
	for(int dan = 1; dan <= 9; dan++) {
		for(int suji = 0x10; suji <= 0x90; suji += 0x10) {
			...
			z = suji + dan;
			board[z] = Piece(bd99[dan-1][9 - suji/0x10]);

			// 駒番号系のデータ設定
#define KNABORT()	fprintf(stderr, "Error!:%s:%d:board[0x%X] == 0x%X\n", __FILE__, __LINE__, z, board[z]);exit(-1)
...
/// ※ position.cppのdo_move()の変更を忘れずに反映する.
void Position::do_move(Move m, StateInfo& newSt)
{
	...
	std::memcpy(&newSt, st, offsetof(StateInfo, board_key));
	newSt.previous = st;
	st = &newSt;
...
//				DelPinInfG(NanohaTbl::Direction[id]);
	...
	if (capture) {
		...
		kn = komano[to];
		knkind[kn] = Piece((capture ^ GOTE) & ~(PROMOTED));
...
#if !defined(TSUMESOLVER)
		// material 更新
		st->material -= NanohaTbl::KomaValueEx[capture];
#endif//#if !defined(TSUMESOLVER)

		// ハッシュ更新
		k ^= Zobrist::psq[capture][to];
		Piece cpc = Piece((capture ^ GOTE) & ~PROMOTED);
		st->hand_key ^= Zobrist::hand[cpc][hand[us].get(PieceType(cpc & ~GOTE))];
	} else {
		// 移動先は空→移動先の長い利きを消す
		// 後手の利きを消す
...
#if !defined(TSUMESOLVER)
		// material 更新
		st->material += NanohaTbl::KomaValuePro[piece];
#endif//#if !defined(TSUMESOLVER)
	...
	// ハッシュ更新
	k ^= Zobrist::psq[board[from]][from] ^ Zobrist::psq[piece][to];

	// Prefetch TT access as soon as we know key is updated
	// ※ k == board_keyなので、hand_keyも加味する.
	prefetch2(reinterpret_cast<char*>(TT.first_entry(k ^ st->hand_key)));
	...
	if (pos_is_ok(&fail) == false) {
		std::cerr << "Error!:pos_is_ok() is false. Reason code = " << fail << std::endl;
		print_csa(m);
		MYABORT();
	}
#else
	...
	StateInfo si;
	compute_key(&si);
	assert(key() == (si.board_key ^ si.hand_key));
	assert(board_key() == si.board_key);
	assert(hand_key() == si.hand_key);
#endif
}
...
void Position::do_drop(Move m)
{
	...
	// Set capture piece
	st->capturedPiece = EMP;

	// Update the key with the final value
	st->board_key ^= Zobrist::psq[piece][to];
	st->hand_key ^= Zobrist::hand[piece][hand[us].get(PieceType(piece & ~GOTE))+1];

	// Prefetch TT access as soon as we know key is updated
	prefetch2(reinterpret_cast<char*>(TT.first_entry(st->board_key ^ st->hand_key)));
...
///				pin[to] = 0;
	...
	del_effect(to, board[to]);					// 動かした駒の利きを消す
...
///		piece &= ~PROMOTED;
	...
	if (capturedPiece) {
#if !defined(TSUMESOLVER)
		// material 更新
		st->material += NanohaTbl::KomaValueEx[capturedPiece];
#endif//#if !defined(TSUMESOLVER)

		int kne = 0;
		switch (capturedPiece & ~(GOTE|PROMOTED)) {
		case EMP:
			break;
		case FU:
			kn  = KNS_FU;
		...
		if (kn > kne) {
			Print();
			move_print(m);
			output_info(":kn=%d, kne=%d, capture=0x%X\n", kn, kne, capturedPiece);
			MYABORT();
		}
#endif
		knkind[kn] = capturedPiece;
		knpos[kn] = to;
		board[to] = capturedPiece;
		komano[to] = kn;
		add_effect(to);	// 取った駒の利きを追加

		if (us == BLACK) handS.dec(capturedPiece & ~(GOTE | PROMOTED));
		else             handG.dec(capturedPiece & ~(GOTE | PROMOTED));
	} else {
		// 移動先は空→移動先の長い利きを通す
		// 後手の利きを消す
		if ((tkiki = effectW[to] & EFFECT_LONG_MASK) != 0) {
	...
	if ((tkiki = effectW[from] & EFFECT_LONG_MASK) != 0) {
		while (tkiki) {
			...
			if (piece == SOU) {
				// 長い利きは玉を一つだけ貫く
				if (board[from + NanohaTbl::Direction[id]] != WALL) effectW[from + NanohaTbl::Direction[id]] |= (1u << id);
			}
		}
	}
	// 先手の利きを消す
	if ((tkiki = effectB[from] & EFFECT_LONG_MASK) != 0) {
		while (tkiki) {
			...
			if (piece == GOU) {
				// 長い利きは玉を一つだけ貫く
				if (board[from + NanohaTbl::Direction[id]] != WALL) effectB[from + NanohaTbl::Direction[id]] |= (1u << id);
			}
		}
	}
...
// 手を進めずにハッシュ計算のみ行う
Key Position::calc_hash_no_move(const Move m) const
{
	uint64_t new_key = board_key();

	new_key ^= Zobrist::side;	// 手番反転

	// from の計算
	int from = from_sq(m);
	int to = to_sq(m);
	int piece = piece_of(m);
	if (!is_drop(m)) {
		// 空白になったことで変わるハッシュ値
		new_key ^= Zobrist::psq[piece][from];
	}

	// to の処理
	// board[to]にあったものをＨａｓｈから消す
	Piece capture = capture_of(m);
	if (capture) {
		new_key ^= Zobrist::psq[board[to]][to];
	}

	// 新しい駒をＨａｓｈに加える
	if (is_promotion(m)) piece |= PROMOTED;
	new_key ^= Zobrist::psq[piece][to];
...
bool Position::move_attacks_square(Move m, Square sq) const
{
	...
	const Piece piece = is_promotion(m) ? Piece(piece_of(m) | PROMOTED) : piece_of(m);
	const Square to = to_sq(m);
...
{
	return is_check_move(sideToMove, m);
}

bool Position::is_evasion(Move m) const
{
	Piece pc = piece_of(m);
	const Color us = sideToMove;
	const Color them = ~us;
	assert(sideToMove == color_of(pc));
	assert(in_check());
	Square to = to_sq(m);
	Square ksq = king_square(us);

	if (type_of(pc) == OU) {
		// 玉を動かすときは、toに相手の利きがなければOK.
		if (effect[them][to] == 0) return true;
	} else {
		// 玉以外を動かす.
		const effect_t efft = effect[them][ksq];
		// 両王手のときは、玉を動かすしかないのでここに来ることはNG.
		if ((efft & (efft - 1)) != 0) return false;
		unsigned long id = 0;	// 初期化不要だが warning が出るため0を入れる
		int check;	// 王手をかけている駒の座標
		if (efft & EFFECT_SHORT_MASK) {
			if (is_drop(m)) return false;
			// 跳びのない利きによる王手 → 回避手段：王手している駒を取る、玉を動かす
			_BitScanForward(&id, efft);
			check = ksq - NanohaTbl::Direction[id];
			// 王手をかけている駒を取る
			// ピンされていることはありえないので、pinが0かどうかだけ確認する.
			if (!is_drop(m) && to == check && pin[from_sq(m)] == 0) return true;
		} else {
			// 跳び利きによる王手 → 回避手段：王手している駒を取る、合駒
			if (!is_drop(m)) {
				_BitScanForward(&id, efft);
				check = SkipOverEMP(ksq, -NanohaTbl::Direction[id]);
				// 王手をかけている駒を取る
				// ピンされていることはありえないので、pinが0かどうかだけ確認する.
				if (pin[from_sq(m)] != 0) return false;
				if (to == check) return true;
			}
			// 合駒の判断
			if (!empty(to)) return false;
			// toが玉の放射線上にいて、玉への利きと同じ利きがあればOKとする(ちょっと手抜き)
			if (relate_pos(to, ksq) != 0 && (effect[them][to] & efft) != 0) return true;
		}
	}
...
bool Position::pseudo_legal(const Move m) const
{
	...
	if (is_drop(m)) {
		...
		if (DEBUG_LEVEL > 0) {
			...
			std::cerr << "   piece=" << int(piece) << ", cap=" << int(capture_of(m)) << std::endl;
			std::cerr << "   board[from]=" << int(board[from]) << ", board[to]=" << int(board[to]) << std::endl;
		}
#endif
		if (board[from] != piece) {
			return false;
		...
		if (board[to] != EMP && color_of(board[to]) == us) {
			// 自分の駒を取っている
			return false;
		}
		// 玉の場合、自殺はできない
		...
		// TODO:駒を飛び越えないか？
		int d = std::max(abs((from >> 4)-(to >> 4)), abs((from & 0x0F)-(to&0x0F)));
		if (pt == KE) {
			if (d != 2) return false;
		} else if (d > 1) {
			// 香、角、飛、馬、龍しかない.
...
ExtMove* Position::add_straight(ExtMove* mlist, const int from, const int dir) const
{
	...
	if (z_pin == 0 || abs(z_pin) == abs(dir)) {
		...
		for (to = from + dir; board[to] == EMP; to += dir) {
			dan = to & 0x0f;
		...
		// 味方の駒でないなら、そこへ動く
		if ((us == BLACK && (board[to] != WALL) && (board[to] & GOTE))
		 || (us == WHITE && (board[to] != WALL) && (board[to] & GOTE) == 0)) {
			dan = to & 0x0f;
			promote |= can_promotion<us>(dan);
			tmp &= ~TO_MASK;
			tmp |= To2Move(to) | Cap2Move(board[to]);
			if (promote && (piece & PROMOTED) == 0) {
				(mlist++)->move = Move(tmp | FLAG_PROMO);
				if (piece == SKY) {
					if (dan > 1) {
...
ExtMove* Position::gen_move_to(const Color us, ExtMove* mlist, int to) const
{
	...
	while (efft) {
		...
		if (pn == 0 || abs(pn) == abs(NanohaTbl::Direction[id])) {
			if (us == BLACK) {
				if (board[z] != SOU) mlist = add_moveB(mlist, z, to - z);
			} else {
				if (board[z] != GOU) mlist = add_moveW(mlist, z, to - z);
			}
		}
	}
	return mlist;
...
///	int teNum = teNumM;	// アドレスを取らない
	...
	if (exists > 0) {
		...
		for (suji = 0x10; suji <= 0x90; suji += 0x10) {
			...
			// 打ち歩詰めもチェック
#define FU_FUNC(z)	\
	if (board[z] == EMP && !is_pawn_drop_mate(us, z)) {	\
...
#undef GI_FUNC
	...
	while (top != mlist) {
		Move m = top->move;
		if (!is_drop(m)) {
			if (piece_on(Square(from_sq(m))) == EMP) {
				assert(false);
			}
		}
		top++;
...
#define MoveKB(dir) to = kingS - DIR_##dir;	\
					if (EXIST_EFFECT(effectW[to]) == 0) {	\
						...
						if (koma == EMP || (koma & GOTE)) {		\
							(mlist++)->move = Move(tmp | To2Move(to) | Cap2Move(board[to]));	\
						}		\
					}
...
ExtMove* Position::generate_capture(ExtMove* mlist) const
{
	...
	for (kno = 1; kno <= MAX_PIECENUMBER; kno++) {
		to = knpos[kno];
		if (OnBoard(to)) {
			if (color_of(Piece(knkind[kno])) == them && EXIST_EFFECT(our_effect[to])) {
				...
				while (k) {
					...
					type = type_of(board[from]);
					if (type == OU) {
						// 玉は相手の利きがある駒を取れない
						if (EXIST_EFFECT(their_effect[to]) == 0) {
							mlist->move = cons_move(from, to, board[from], board[to], 0);
							mlist++;
						}
						...
						if (type == GI) {
							// 銀は成と不成を生成する
							mlist->move = cons_move(from, to, board[from], board[to], 1);
							mlist++;
							mlist->move = cons_move(from, to, board[from], board[to], 0);
							mlist++;
						} else if (type == FU) {
						...
						// 成れない位置
						mlist->move = cons_move(from, to, board[from], board[to]);
						mlist++;
					}
				}
	...
	for (kno = KNS_FU; kno <= KNE_FU; kno++) {
		if (knkind[kno] == make_piece(us, FU)) {
			...
			if (OnBoard(from) && can_promotion<us>(to) && board[to] == EMP) {
...
ExtMove* Position::generate_non_capture(ExtMove* mlist) const
{
	...
	from = sq_king<us>();	// 玉
	if (from) mlist = gen_king_noncapture(us, mlist);	// 詰将棋など玉がない時を考慮
	...
	for (mlist = p; mlist < last; mlist++) {
		...
		if (capture_of(tmp) != EMP) {
			if (is_promotion(tmp)) continue;
			// ここで、取る手&&成らない手になっている。
			PieceType pt = ptype_of(tmp);
			switch (pt) {
			case FU:
				if (can_promotion<us>(to_sq(tmp))) break;
				continue;
			case KE:
			case GI:
			case KI:
			...
			case KY:
				...
				if (((us == BLACK && (to_sq(tmp) & 0x0F) != 2) || (us == WHITE && (to_sq(tmp) & 0x0F) != 8))) continue;
				break;
			case KA:
			case HI:
				// 角飛は成らない手を生成していないので、成れるのに成らない手はすべて対象にする
				if (!can_promotion<us>(to_sq(tmp)) && !can_promotion<us>(from_sq(tmp))) continue;
				break;
			case NO_PIECE_TYPE:
			default:
				print_csa(tmp);
		...
		// 飛車が敵陣に成り込む手は生成済み
		if (ptype_of(tmp) == HI && is_promotion(tmp) && !can_promotion<us>(from_sq(tmp)) && can_promotion<us>(to_sq(tmp))) continue;
		// 歩の成る手は生成済み
		if (ptype_of(tmp) == FU && is_promotion(tmp)) continue;
...
ExtMove* Position::generate_evasion(ExtMove* mlist) const
{
	...
	if ((efft & (efft - 1)) != 0) {
		...
		if (efft & EFFECT_SHORT_MASK) {
			...
			//合駒をする手を生成する
			int sq;
			for (sq = ksq - NanohaTbl::Direction[id]; board[sq] == EMP; sq -= NanohaTbl::Direction[id]) {
...
bool Position::IsKachi(const Color us) const
{
	...
	if (us == BLACK) {
		...
		for (suji = 0x10; suji <= 0x90; suji += 0x10) {
			for (dan = 1; dan <= 3; dan++) {
				Piece piece = Piece(board[suji+dan] & ~PROMOTED);		// 玉は0になるのでカウント外
		...
		for (suji = 0x10; suji <= 0x90; suji += 0x10) {
			for (dan = 7; dan <= 9; dan++) {
				Piece piece = Piece(board[suji+dan] & ~PROMOTED);
...
//   次の記録開始bit位置
//
int get_bit_board(const int start_bit, int& data, const unsigned char buf[], const int size)
{
	unsigned base;
	int bits;
	const int start_bytes = (start_bit >> 3);
	const int shift_bits = start_bit & 7;
	assert(start_bytes < size);

	// start_bitから8bit分をbaseに抽出する.
	if (shift_bits != 0) {
		base = (buf[start_bytes] >> shift_bits);
		if (start_bytes < size-1) {
			base |= (buf[start_bytes+1] << (7-shift_bits)) & 0xFF;
		}
	} else {
		base = buf[start_bytes];
	}
	...
	// xx0011 香.
	else if ((base & 0x0F) == 0x03) {
		data = KY;
		bits = 6;
	}
	// xx1011 桂.
	else if ((base & 0x0F) == 0x0B) {
		data = KE;
		bits = 6;
	}
	// xx0111 銀.
	else if ((base & 0x0F) == 0x07) {
		data = GI;
		bits = 6;
	}
	// x01111 金.
	else if ((base & 0x0F) == 0x0F) {
		data = KI;
		bits = 6;
		if (base & 0x20) data |= GOTE;
		return start_bit + bits;
	}
	// 011111 角.
	else if ((base & 0x3F) == 0x1F) {
		data = KA;
		bits = 8;
	}
	// 111111 飛.
	else if ((base & 0x3F) == 0x3F) {
		data = HI;
		bits = 8;
	} else {
		// Error
	...
	base >>= (bits - 2);
	if (base & 1) data |= PROMOTED;
	if (base & 2) data |= GOTE;
	assert(start_bit + bits <= 8*size);
	return start_bit + bits;
}
...
//   次の記録開始bit位置
//
int get_bit_hand(const int start_bit, int& data, const unsigned char buf[], const int size)
{
	unsigned base;
	int bits;
	const int start_bytes = (start_bit >> 3);
	...
	// xx101 桂.
	else if ((base & 0x07) == 0x05) {
		data = KE;
		bits = 4;
	}
	// xx011 銀.
	else if ((base & 0x07) == 0x03) {
		data = GI;
		bits = 4;
	}
	// x0111 金.
	...
	// 01111 角.
	else if ((base & 0x1F) == 0x0F) {
		data = KA;
		bits = 6;
	}
	// 11111 飛.
	else if ((base & 0x1F) == 0x1F) {
		data = HI;
		bits = 6;
	} else {
		// Error
...
#if !defined(NDEBUG)
	Position pos;
	pos.DecodeHuffman(buf);
	if (*this != pos) {
		// エラー
		std::cerr << "Error!:Huffman encode and decode." << std::endl;
		print_csa();
		exit(1);
	}
#endif
...
//   正の値：エンコードしたときのビット数
//
int Position::DecodeHuffman(unsigned char buf[32])
{
	constexpr int size = 32;	// buf[] のサイズ
	int z, to;
	size_t i;
	...
	// エラーチェック.
	z = kingS;
	if (z > 0x99 || 0x11 > z) return -1;
	if ((z & 0x0F) > 9 || 1 > (z & 0x0F)) return -1;
	z = kingG;
	if (z > 0x99 || 0x11 > z) return -1;
	if ((z & 0x0F) > 9 || 1 > (z & 0x0F)) return -1;
	...
	for (suji = 0x10; suji <= 0x90; suji += 0x10) {
		for (dan = 1; dan <= 9; dan++) {
			piece = board[suji + dan];
			if (piece == SOU || piece == GOU) continue;
			start_bit = get_bit_board(start_bit, data, buf, size);
			n++;
		}
	}
	...
	// 持駒を復号
	while (n < 38) {
		start_bit = get_bit_hand(start_bit, data, buf, size);
		if (data & GOTE) {
			hand[1].inc(data & ~GOTE);
		} else {
			hand[0].inc(data);
		}
		n++;
