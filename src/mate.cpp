...
ExtMove* Position::gen_check_long(ExtMove* mlist) const
{
	...
	for (int hi = 0; hi < 2; hi++) {
		...
		// 相手の駒は処理しない
		if (color_of(board[sq]) != us) continue;
		...
		if (suji == ksuji || dan == kdan) {
			...
			if (sq == SkipOverEMP(to, dir)) {
				if (color_of(board[to]) != us) {
					// toにいる駒は敵
					if ((pin[sq]==0 || pin[sq]==dir || pin[sq]==-dir)) {
						if ((board[sq] & PROMOTED) == 0 && (can_promotion<us>(to) || can_promotion<us>(sq))) {
							// 成る手
							(mlist++)->move = cons_move(sq, to, board[sq], board[to], 1, MOVE_CHECK_LONG);
							// 不成り
							(mlist++)->move = cons_move(sq, to, board[sq], board[to], 0, MOVE_CHECK_LONG | MOVE_CHECK_NARAZU);
						} else {
							// 不成り
							(mlist++)->move = cons_move(sq, to, board[sq], board[to], 0, MOVE_CHECK_LONG);
						}
					}
				} else {
					// toにいる駒は味方
			...
			int xmin = std::min(ksuji, suji);
			int xmax = std::max(ksuji, suji);
			int ymin = std::min(kdan, dan);
			int ymax = std::max(kdan, dan);
			int x, y;
			bool moveH, moveV;
			moveH = moveV = true;
			for (x = xmin + 16; x < xmax; x += 16) {
				if (board[x +  dan] != EMP) moveH = false;	// 飛の横方向に駒があれば、飛が横に動く王手はない.
				if (board[x + kdan] != EMP) moveV = false;	// 玉の横方向に駒があれば、飛が縦に動く王手はない.
			}
			for (y = ymin + 1; y < ymax; y++) {
				if (board[ksuji + y] != EMP) moveH = false;	// 玉の縦方向に駒があれば、飛が横に動く王手はない.
				if (board[suji  + y] != EMP) moveV = false;	// 飛の縦方向に駒があれば、飛が縦に動く王手はない.
			}
			// 飛車が横に動く王手
			if (moveH) {
				if ((board[ksuji + dan] == EMP || color_of(board[ksuji + dan]) != us) 
					&& (pin[sq] == 0 || pin[sq] == 16 || pin[sq] == -16)) {
					if ((board[sq] & PROMOTED) == 0 && can_promotion<us>(sq)) {
						(mlist++)->move = cons_move(sq, ksuji + dan, board[sq], board[ksuji + dan], 1, MOVE_CHECK_LONG);
						(mlist++)->move = cons_move(sq, ksuji + dan, board[sq], board[ksuji + dan], 0, MOVE_CHECK_LONG | MOVE_CHECK_NARAZU);
					} else {
						(mlist++)->move = cons_move(sq, ksuji + dan, board[sq], board[ksuji + dan], 0, MOVE_CHECK_LONG);
					}
				}
			}
			// 飛車が縦に動く王手
			if (moveV) {
				if ((board[suji + kdan] == EMP || color_of(board[suji + kdan]) != us)
					&& (pin[sq] == 0 || pin[sq] == 1 || pin[sq] == -1)) {
					if ((board[sq] & PROMOTED) == 0 && (can_promotion<us>(sq) || can_promotion<us>(enemyKing))) {
						(mlist++)->move = cons_move(sq, suji + kdan, board[sq], board[suji + kdan], 1, MOVE_CHECK_LONG);
						(mlist++)->move = cons_move(sq, suji + kdan, board[sq], board[suji + kdan], 0, MOVE_CHECK_LONG | MOVE_CHECK_NARAZU);
					} else {
						(mlist++)->move = cons_move(sq, suji + kdan, board[sq], board[suji + kdan], 0, MOVE_CHECK_LONG);
					}
				}
			}
			// 竜が斜めに動く手を生成する
			if (board[sq] & PROMOTED)
			for (int dx = -0x10; dx <= 0x10; dx += 0x20) {
				for (int dy = -1; dy <= 1; dy += 2) {
					if (pin[sq] != 0 && pin[sq] != dx + dy && pin[sq] != -dx - dy) continue;
					if (board[sq+dx+dy] == WALL || (board[sq+dx+dy] != EMP && color_of(board[sq + dx + dy]) == us)) continue;	// 味方の駒
					if (kdan == dan + dy) {
						//斜めに動いて横に王手
						int sx = std::min(ksuji, suji + dx);
						int lx = std::max(ksuji, suji + dx);
						moveV = true;
						for (x = sx + 16; x < lx; x += 16) {
							if (board[x + kdan] != EMP) moveV = false;
						}
						if (moveV) {
							(mlist++)->move = cons_move(sq, sq + dx + dy, board[sq], board[sq + dx + dy], 0, MOVE_CHECK_LONG);
						}
					}
					if (ksuji == suji + dx) {
						//斜めに動いて縦に王手
						int sy = std::min(kdan, dan + dy);
						int ly = std::max(kdan, dan + dy);
						moveH = true;
						for (y = sy + 1; y < ly; y++) {
							if (board[y + ksuji] != EMP) moveH = false;
						}
						if (moveH) {
							(mlist++)->move = cons_move(sq, sq + dx + dy, board[sq], board[sq + dx + dy], 0, MOVE_CHECK_LONG);
						}
			...
			// 3. 飛が成ること及び竜による王手
			{
				...
				for (i = 0; i < 4; i++) {
					...
					if (board[to] != EMP && (board[to] == WALL || color_of(board[to]) == us)) {	// 味方の駒
						continue;
					}
					for (j = 0; j < 4; j++) {
						dir = dirs[j];
						if (sq == SkipOverEMP(to, dir)) {
							if ((board[sq] & PROMOTED) && (pin[sq] == 0 || pin[sq] == dir || pin[sq] == -dir)) {
								(mlist++)->move = cons_move(sq, to, board[sq], board[to], 0);
							} else if ((board[sq] & PROMOTED) == 0 && (pin[sq] == 0 || pin[sq] == dir || pin[sq] == -dir)
							  && ((can_promotion<us>(sq) || can_promotion<us>(to)))
								 ) {
								(mlist++)->move = cons_move(sq, to, board[sq], board[to], 1);
							}
						}
					}
				}
		...
		if (k1 != b1 && k2 != b2) {
			...
			if ((k1 & 1) == (b1 & 1)) {
				...
				if ((x1 >= 1) && (x1 <= 9) && (y1 >= 1) && (y1 <= 9)
					...
					&& (board[to = ((x1 << 4)+y1)] == EMP || color_of(board[to]) != us)) {
					bool moveF = true;
					int s = std::min(sq, to);
					int l = std::max(sq, to);
					int j;
					for (j = s + dir; j < l; j += dir) {
						if (board[j] != EMP) moveF = false;
					}
					s = std::min(to, enemyKing);
					l = std::max(to, enemyKing);
					dir = 15;
					for (j = s + dir; j < l; j += dir) {
						if (board[j] != EMP) moveF = false;
					}
				...
				if (board[sq] & PROMOTED) {
					...
					for (int dirID = 0; dirID < 4; dirID++) {
						...
						if (board[sq + dirs[dirID]] == WALL || (board[sq + dirs[dirID]] != EMP && color_of(board[sq + dirs[dirID]]) == us)) continue;		// 味方の駒
						if (k1 == b1 + p[dirID]) {
							//動いてp方向に王手
							int s = std::min(enemyKing, sq + dirs[dirID]);
							int l = std::max(enemyKing, sq + dirs[dirID]);
							bool moveF = true;
							for (int j = s + 15; j < l; j += 15) {
								if (board[j] != EMP) moveF = false;
							}
							if (moveF) {
								(mlist++)->move = cons_move(sq, sq + dirs[dirID], board[sq], board[sq + dirs[dirID]], 0, MOVE_CHECK_LONG);
							}
						}
						if (k2 == b2 + m[dirID]) {
							//動いてm方向に王手
...
///			}
			...
			if (to != 0) {
				int from = to + dir;
				if (board[to] != EMP && color_of(board[to]) == us) {	// 味方の駒
					to = from;
					from += dir;
				} else if (board[from] != EMP && color_of(board[from]) != us) {	// 敵の駒
					to = from;
					from += dir;
				}
				while (board[from] == EMP) {
					from += dir;
				}
				if (from == sq && (pin[sq] == 0 || pin[sq] == dir || pin[sq] == -dir)) {
					if ((us == BLACK) && ((to & 0x0f) <= 3)) {
						...
						if ((to & 0x0F) == kdan && kdan <= 2 && (sq & 0x0F) != kdan + 1) {
							(mlist++)->move = cons_move(sq, to + dir, board[sq], board[to+dir], 1);
						}
					}
...
ExtMove* Position::gen_check_short(ExtMove* mlist) const
{
	...
	if (us == BLACK) {
		...
		while (OuteMove2[++i].from) {
			...
			if (board[from] == WALL) continue;
			const int koma = board[from];
			if (koma != EMP && IsSente(koma)) {
				j = 0;
				while (OuteMove2[i].to[j].to) {
					int to = enemyKing + OuteMove2[i].to[j].to;
					if (board[to] != WALL && (board[to] == EMP || IsGote(board[to]))) {
		...
		while (OuteMove2[++i].from) {
			...
			if (koma != EMP && IsGote(koma)) {
				...
				while (OuteMove2[i].to[j].to) {
					int to = enemyKing - OuteMove2[i].to[j].to;
					if (board[to] != WALL && (board[to] == EMP || IsSente(board[to]))) {
						if (pin[from] == 0 || pin[from] == to-from || pin[from] == from-to) {
...
ExtMove* Position::gen_check_drop(ExtMove* mlist, bool& bUchifudume) const
{
	...
	if (us == BLACK) {
		...
		if (handS.getKE() > 0 && kdan <= 7) {
			piece = SKE;
			if (board[enemyKing - 14] == EMP) {
				(mlist++)->move = cons_move(0, enemyKing - 14, piece, EMP);
			}
			if (board[enemyKing + 18] == EMP) {
		...
		suji = ksuji;
		dan = kdan + 1;
		if (handS.getKY() > 0 && board[dan+suji] == EMP) {
			for (; dan <= 9; dan++) {
				if (board[dan+suji]==EMP) {
		...
		suji = ksuji;
		dan = kdan + 1;
		if (handS.getFU() > 0 && board[dan+suji] == EMP) {
			// 二歩チェック
	...
	if (us != BLACK) {
		...
		suji = ksuji;
		dan = kdan - 1;
		if (handG.getKY() > 0 && board[dan+suji] == EMP) {
			for (; dan >= 1; dan--) {
				if (board[dan+suji]==EMP) {
		...
		suji = ksuji;
		dan = kdan - 1;
		if (handG.getFU() > 0 && board[dan+suji] == EMP) {
			// 二歩チェック
			int nifu = is_double_pawn(us, suji);
			if (nifu == 0) {
				// 打ち歩詰めもチェック
...
ExtMove* Position::gen_check_drop3(ExtMove* mlist, bool& bUchifudume) const
{
	...
	Piece piece;
	// 金を打つ
#define ADD_DROP(x)  if (board[enemyKing + (x)] == EMP) (mlist++)->move = cons_move(0, enemyKing + (x), piece, EMP)
	if (((us == BLACK) ? handS.getKI() : handG.getKI()) > 0) {
		if (us == BLACK) {
			piece = SKI;
			ADD_DROP(  1);
...
ExtMove* Position::generate_check(ExtMove* mlist, bool& bUchifudume) const
{
	...
	if (last != mlist) {
		...
		for (cur = &top[1]; cur != last; cur++) {
			bool flag = false;
			for (j = 0; j < teNum; j++) {
				if (equal_move(top[j].move, cur->move)) {
					flag = true;
					break;
				}
			}
...
MYABORT();
			...
			if (kingPos - NanohaTbl::Direction[id] != check) {
				int i;
				for (i = kingPos - NanohaTbl::Direction[id]; board[i] == EMP; i -= NanohaTbl::Direction[id]) {
...
int Position::Mate3(const Color us, Move &m)
{
	...
	if (probe_m3hash(*this, m)) {
		M3Perform::hashhit++;
		return is_none(m) ? -VALUE_MATE : VALUE_MATE;
	}
#endif

	ExtMove moves[256]; 	// 深さ3程度なら十分な大きさ
	ExtMove *cur, *last;
...
template ExtMove* Position::generate_check3<BLACK>(ExtMove* mlist, bool& bUchifudume) const;
template ExtMove* Position::generate_check3<WHITE>(ExtMove* mlist, bool& bUchifudume) const;
