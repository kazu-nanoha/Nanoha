...
uint32_t Position::infoRound8King() const
{
	...
	int defKPos = (us == BLACK) ? king_square(WHITE) : king_square(BLACK);
...
uint32_t Position::chkInterceptMove(const uint32_t info, const int kpos, const int i, const int from, const int kind) const
{
	...
	while (kiki) {
		...
		while (check) {
			...
			if (z2 == from) {
				// 動かす駒の利きしかない
				if (CanAttack(kind, to, z) == 0) return 1;
			}
		}
	}
	return ret;
...
//	if ((info & 0xFFFF) == 0) return 0;
	...
	int enemyKing = (us == BLACK) ? king_square(WHITE)   : king_square(BLACK);
...
// 位置 from の駒 kind は位置 to に利くか?
// [戻り値]
//    int		検証結果：0(利かない)、Non-Zero(利く)
int Position::CanAttack(const int kind, const int from2, const int to) const
{
	switch (kind) {
	case EMP:
	case SFU:
	...
	case 2*DIR_RIGHT:
		if (tbl[5] & (1u << kind)) {
			mid = to - diff2 / 2;
			if (board[mid] == EMP) return 1;
			if ((board[mid] & ~GOTE) == OU) return 1;
		}
		break;
	case 2*DIR_UR:
	case 2*DIR_UL:
...
uint32_t Position::CheckMate1plyMove(const uint32_t info, Move &m, const int check) const
{
	...
	int mNum;
	Move mBuf[32];
	const unsigned int enemyKing = (us == BLACK) ? king_square(WHITE)   : king_square(BLACK);	// 玉方の玉の位置
	const int SorG = (us == BLACK) ? SENTE : GOTE;
	...
	if ((info & 0x0000FF00) == 0) {
		...
		if (check == 0 || to == check) {
			if ((aKiki[to] & 0x00000F00) && board[to] != WALL && (board[to] == EMP || (board[to] & GOTE) != SorG)) {
				...
				if (bCheck) {
					...
					from = to + d1;
					if (board[from] == (SorG | KE) && pin[from] == 0 &&  chkInterceptMove<us>(info, enemyKing, kdir1, from, SorG | KE) == 0) {
						m = cons_move(from, to, board[from], board[to], 0);
						return 1;
					}
					from = to + d2;
					if (board[from] == (SorG | KE) && pin[from] == 0 && chkInterceptMove<us>(info, enemyKing, kdir1, from, SorG | KE) == 0) {
		...
		if (check == 0 || to == check) {
			if ((aKiki[to] & 0x00000F00) && board[to] != WALL && (board[to] == EMP || (board[to] & GOTE) != SorG)) {
				...
				if (bCheck) {
					...
					// ここでの kdir1 or kdir2 の指定は上の to の計算に合わせる
					from = to + d1;
					if (board[from] == (SorG | KE) && pin[from] == 0 && chkInterceptMove<us>(info, enemyKing, kdir2, from, SorG | KE) == 0) {
						m = cons_move(from, to, board[from], board[to], 0);
						return 1;
					}
					from = to + d2;
		...
		while (kiki) {
			...
			case GHI:
				...
				if (bCanPromote && (TblKikiCheck[kind | PROMOTED] & (1u << id)) != 0) {
					if (TblMate1plydrop[info24] & ((1u << PROMOTED ) << kind))
						if (chkInterceptMove<us>(info, enemyKing, id, from, kind | PROMOTED) == 0) { mBuf[mNum++] = cons_move(from, to, Piece(kind), board[to], 1); }
				} else if ((TblKikiCheck[kind] & (1u << id)) != 0) {
					if (TblMate1plydrop[info24] & ((1u             ) << kind))
						if (chkInterceptMove<us>(info, enemyKing, id, from, kind           ) == 0) { mBuf[mNum++] = cons_move(from, to, Piece(kind), board[to], 0); }
				}
				break;
			case SKY:
			case GKY:
			...
			if (mNum > 0) {
				...
				while (info3) {
					...
					if (k == 0) {
						// 利きがない場合、移動する駒が利くか?
						if (CanAttack(kd, to, z) == 0) {
							bEscape1 = true;
						}
						if (mNum < 2 || (CanAttack(kind, to, z) == 0)) {
							bEscape2 = true;
						}
...
#define CHK_MATE1PLY(m)
	...
	effect_t kiki = (us == BLACK)
		 ? ( (king_square(BLACK) == 0) ? 0 : EXIST_EFFECT(effectW[king_square(BLACK)]) )
		 : ( (king_square(WHITE) == 0) ? 0 : EXIST_EFFECT(effectB[king_square(WHITE)]) );
	if (kiki == 0) {
		// 王手がかかっていない
		// 移動による一手詰の判定
		...
		if (kiki & EFFECT_SHORT_MASK) {
			check = (us == BLACK) ? king_square(us) - NanohaTbl::Direction[id] : king_square(WHITE) - NanohaTbl::Direction[id];
		} else {
			check = (us == BLACK) ? SkipOverEMP(king_square(BLACK), -NanohaTbl::Direction[id]) : SkipOverEMP(king_square(WHITE), -NanohaTbl::Direction[id]);
		}
