...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, Search::Stack* s)
  ...
  countermove = pos.this_thread()->counterMoves[pos.piece_on(prevSq)][prevSq];
	...
	if (stage == EVASION) {
		ttMove = (ttm && pos.pseudo_legal(ttm) && pos.is_evasion(ttm)) ? ttm : MOVE_NONE;
	} else {
		ttMove = ttm && pos.pseudo_legal(ttm) ? ttm : MOVE_NONE;
	}
  stage += is_none(ttMove);
#else
...
void MovePicker::score<CAPTURES>() {
  ...
  for (auto& m : *this) {
#if defined(NANOHA)
		// 段で点を変えるのは将棋に合わなさそうなので、MVV/LVA的な付け方をする。
		const Piece pc = piece_of(m);
		m.value = PieceValue[MG][pos.piece_on(to_sq(m))] - type_of(pc);
		if (is_promotion(m)) {
			// [ToDo] 成りの価値を足す
			m.value += (PieceValue[MG][(Piece(pc | PROMOTED))] >> 1);
		}
#else
...
void MovePicker::score<QUIETS>() {
  ...
  for (auto& m : *this) {
#if defined(NANOHA)
		const Piece pc = is_promotion(m) ? Piece(piece_of(m) | PROMOTED) : piece_of(m);
		m.value =  cmh[pc][to_sq(m)]
		         + fmh[pc][to_sq(m)]
		         + fm2[pc][to_sq(m)]
		         + history.get(c, m);
#else
...
#if defined(NANOHA)
          m.value =  PieceValue[MG][pos.piece_on(to_sq(m))]
                   - Value(ptype_of(m)) + HistoryStats::Max;
#else
          m.value =  PieceValue[MG][pos.piece_on(to_sq(m))]
