...
#if defined(NANOHA)
//
// 汎用バージョン(CAPTURES, NON_EVASIONS, QUIETS を想定)
//
...
ExtMove* generate(const Position& pos, ExtMove* moveList)
{
	...
	if (Type == NON_EVASIONS) {
		moveList = (us == BLACK)
			? pos.generate_non_evasion<BLACK>(moveList)
			: pos.generate_non_evasion<WHITE>(moveList);
	} else if (Type == CAPTURES) {
		moveList = (us == BLACK)
			? pos.generate_capture<BLACK>(moveList)
			: pos.generate_capture<WHITE>(moveList);
	} else if (Type == QUIETS) {
		moveList = (us == BLACK)
			? pos.generate_non_capture<BLACK>(moveList)
			: pos.generate_non_capture<WHITE>(moveList);
	} else {
		assert(false);
	}
...
#if defined(NANOHA)
template<>
ExtMove* generate<CHECKS>(const Position& pos, ExtMove* moveList)
{
	assert(pos.pos_is_ok());
	assert(!pos.in_check());
	...
	moveList = (us == BLACK) ? pos.generate_check<BLACK>(moveList, bUchifudume)
	                      : pos.generate_check<WHITE>(moveList, bUchifudume);
...
{
	ExtMove *last, *cur = moveList;
	last = generate<CHECKS>(pos, moveList);

	// Remove capture moves from the list
	while (cur != last) {
		if (capture_of(cur->move) == EMP) {
			(moveList++)->move = cur->move;
		}
		cur++;
	}
...
ExtMove* generate<EVASIONS>(const Position& pos, ExtMove* moveList)
{
	...
	return (us == BLACK) ? pos.generate_evasion<BLACK>(moveList) : pos.generate_evasion<WHITE>(moveList);
}
#else
template<>
ExtMove* generate<QUIET_CHECKS>(const Position& pos, ExtMove* moveList) {
...
#if defined(NANOHA)
	return pos.in_check() ? generate<EVASIONS>(pos, moveList)
